...
int init_common_variables() {
  ...
  // 初始化全局mutex和condition || 设置部分全局变量的默认值
  ...
  */
  ...
  /*
    ...
    to that list.
  */
  // 将全局变量status_vars中的系统 status存入all_status_vars中
  ...
  */
  ...
  // 设置默认的身份验证插件(通常为native_password)
  ...
  // 设置mysql版本的后缀(-embedded、-log、-debug...)
  set_server_version();
...
static int init_server_components() {
  ...
  */
  ...
  // 将handler模块的错误信息注册到mysqld的my_error中
  ...
  */
  ...
  // tc_log打开binlog，主要用于binlog和存储引擎的recovery使用，打开当前index中最后一个文件
  ...
  */
  ...
  if (opt_bin_log) {
    ...
    */
    ...
    // mysql_bin_log打开binlog，生成新的binlog，在index文件中加入新的内容
...
#endif
{
  ...
  */
...
#ifndef _WIN32
#ifdef WITH_PERFSCHEMA_STORAGE_ENGINE
  // 初始化performance shcema相关内容
  pre_initialize_performance_schema();
#endif /*WITH_PERFSCHEMA_STORAGE_ENGINE */
  // For windows, my_init() is called from the win specific mysqld_main
  // 初始化 my_sys 和 pthreads
  ...
  // //初始化系统变量 初始化系统变量hash桶，将所有的系统变量插入到hash桶中(这里的变量为sys_vars.cc中定义的变量)
  sys_var_init();
...
#endif /* WITH_PERFSCHEMA_STORAGE_ENGINE */
  // 初始化部分变量，为mysqld初始化系统表等做装备
  heo_error = handle_early_options();
  // 初始化命令，为后续status统计操作数量做准备
  ...
  */
  ...
  // 初始化audit plugin锁，为后续初始化audit plugin做准备
  ...
  */
  ...
  // 所有变量相关内容初始化，包括变量本身的值以及部分变量控制的特性
  ...
  */
  ...
  // 初始化用户名、密码等信息缓存，并将user表中的内容读取缓存
  ...
  */
  ...
  if (!opt_initialize) {
    ...
    */
    ...
    /*
      初始化slave相关的结构体，处理同步相关 init_replica() must be called after the thread keys are created.
    */
    if (server_id != 0)
